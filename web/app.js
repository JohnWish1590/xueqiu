// app.js 强化修复版：
// - 默认显示“时间线”（所有用户），时间越晚越靠前
// - 顶部时间线按钮一键切回所有用户；用户下拉支持桌面/移动端
// - 若 byUser 为空，从 timeline 动态构造用户列表，避免下拉无选项
// - 昵称优先使用 web/nicknames.json（覆盖），再用数据中的 user_name
// - 图片：正文与链接中的缩略图(!thumb.jpg)行内显示；点击缩略图或链接，浮窗显示原图(!raw.jpg)
// - 引用“回复@…”自动灰底块
// - 数据加载带缓存破坏参数 v，避免手机端不刷新
...
(async function(){
  ...
  let deployedAt = null;
  try {
    const build = await (await fetch(`./build.json?v=${ts}`)).json();
    if (build && build.deployedAt) deployedAt = new Date(build.deployedAt);
  } catch {}
  if (!deployedAt) {
    const latest = timeline.reduce((m,i)=>{
  ...
  let overrides = {};
  try {
    overrides = await (await fetch(`./nicknames.json?v=${ts}`)).json();
  } catch {}
  ...
  Object.keys(overrides).forEach(k=>{
    const uid = Number(k);
    const nm = String(overrides[k]||'').trim();
    if (nm) idName.set(uid, nm);
  });
  Object.keys(byUser).forEach(k=>{
    const uid = Number(k);
    const arr = byUser[k];
    const nm = arr && arr[0] && arr[0].user_name ? String(arr[0].user_name).trim() : '';
    if (nm && !idName.has(uid)) idName.set(uid, nm);
  });
  timeline.forEach(it=>{
    const uid = Number(it.user_id);
    const nm = it.user_name ? String(it.user_name).trim() : '';
    if (nm && !idName.has(uid)) idName.set(uid, nm);
  });
  const nameOf = (uid)=>{
    const nm = idName.get(Number(uid));
    return nm && nm.trim() ? nm : `用户${uid}`;
  };

  // 构造用户列表：优先 byUser；若为空，按 timeline 唯一 user_id 构造
  let userListArr = Object.keys(byUser).map(k=>Number(k));
  if (!userListArr.length) {
    const s = new Set();
    timeline.forEach(i=>{ if (Number.isFinite(Number(i.user_id))) s.add(Number(i.user_id)); });
    userListArr = Array.from(s);
  }
  // 生成用户对象并按昵称排序
  const users = userListArr.map(uid=>({ id: uid, name: nameOf(uid) }))
  ...
  if (userListEl && userTpl) {
    ...
    users.forEach(u=>{
      ...
      a.href = `https://xueqiu.com/u/${u.id}`;
  ...
  if (userSelect) {
    userSelect.innerHTML = '<option value="all">全部用户</option>' + users.map(u=>`<option value="${u.id}">${u.name}</option>`).join('');
    userSelect.addEventListener('change', ()=>{
      const val = userSelect.value;
      render({ userId: val === 'all' ? null : Number(val) });
    });
  }

  // 顶部“时间线”按钮：一键切回所有用户（按时间排序）
  ...
  // 引用块：将“回复@…：”包裹为灰底
  function wrapQuotes(body){
    let html = body.innerHTML;
    html = html.replace(/(回复@[^：<]+：[^[<]]*)(<br\s*\/?|$)/g, '<div class="quote">$1</div>$2');
    body.innerHTML = html;
  }
  ...
  const showImg = (idx)=>{ lightboxContent.innerHTML = `<img src="${currentImgs[idx]}" alt="image">`; };
  ...
  function render({ userId = null } = {}){
    // 顶部选择优先生效；userId = null 表示“时间线”（所有用户）
    const topSel = $('#userSelect');
    if (userId === null && topSel && topSel.value && topSel.value !== 'all') {
      userId = Number(topSel.value);
    }
    ...
    list.forEach(item => {
      ...
      nick.textContent = nameOf(item.user_id);
      ...
      // 收集图片并附加缩略图；点击或链接一律开原图（!raw.jpg）
      const links = extractAllImageLinks(body);
      if (links.length) {
        const raws = links.map(href => getThumbAndRaw(href).raw);
        links.forEach(href => {
          const pair = getThumbAndRaw(href);
          // 行内缩略图
          const img = document.createElement('img');
          img.className = 'inline-img';
          img.src = pair.thumb;
          img.addEventListener('click', () => openLightbox([pair.raw]));
          body.appendChild(img);
        });
        // 拦截“查看图片”类链接
        body.querySelectorAll('a').forEach(a => {
          const href = a.getAttribute('href') || '';
          if (href.includes('xqimg.imedao.com')) {
            const raw = getThumbAndRaw(href).raw;
      ...
      const cc = Number(item.comments_count || 0), lc = Number(item.likes_count || 0);
      ...
      art.addEventListener('click', (e) => {
        if (!(e.target.closest('a') || e.target.closest('img.inline-img'))) {
          const s = getRead(); s.add(item.hash); setRead(s); art.classList.remove('unread'); art.classList.add('read');
        }
      });
